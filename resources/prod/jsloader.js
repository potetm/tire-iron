...
/**
 * This is a direct copy of the older goog.net.jsloader. They made breaking
 * changes that don't affect the functionality of tire iron. Instead of
 * trying to maintain compatibility across versions, I'm going to provide
 * my own copy in my own namespace.
 *
...
goog.provide('goog.tire_iron_.jsloader');
goog.provide('goog.tire_iron_.jsloader.Error');
goog.provide('goog.tire_iron_.jsloader.ErrorCode');
goog.provide('goog.tire_iron_.jsloader.Options');
 ...
 * @private {string}
 */
goog.tire_iron_.jsloader.GLOBAL_VERIFY_OBJS_ = 'closure_verification';
 ...
 * @type {number}
 */
goog.tire_iron_.jsloader.DEFAULT_TIMEOUT = 5000;


/**
 * Optional parameters for goog.tire_iron_.jsloader.send.
 ...
 * }}
 */
goog.tire_iron_.jsloader.Options;
 ...
 */
goog.tire_iron_.jsloader.scriptsToLoad_ = [];
 ...
 */
goog.tire_iron_.jsloader.scriptLoadingDeferred_;
 ...
 * matter, you should just call goog.tire_iron_.jsloader.load N times.
 *
 ...
 * @param {goog.tire_iron_.jsloader.Options=} opt_options Optional parameters. See
 *     goog.tire_iron_.jsloader.options documentation for details.
 * @return {!goog.async.Deferred} The deferred result, that may be used to add
 *     callbacks
 */
goog.tire_iron_.jsloader.loadMany = function(uris, opt_options) {
  ...
  var isAnotherModuleLoading = goog.tire_iron_.jsloader.scriptsToLoad_.length;
  goog.array.extend(goog.tire_iron_.jsloader.scriptsToLoad_, uris);
  if (isAnotherModuleLoading) {
    ...
    // finish loading.
    return goog.tire_iron_.jsloader.scriptLoadingDeferred_;
  }

  uris = goog.tire_iron_.jsloader.scriptsToLoad_;
  var popAndLoadNextScript = function() {
    var uri = uris.shift();
    var deferred = goog.tire_iron_.jsloader.load(uri, opt_options);
    if (uris.length) {
      deferred.addBoth(popAndLoadNextScript);
    }
    return deferred;
  };
  goog.tire_iron_.jsloader.scriptLoadingDeferred_ = popAndLoadNextScript();
  return goog.tire_iron_.jsloader.scriptLoadingDeferred_;
};
 ...
 *     The error callback will be called with a single goog.tire_iron_.jsloader.Error
 *     parameter.
 */
goog.tire_iron_.jsloader.load = function(uri, opt_options) {
  var options = opt_options || {};
  var doc = options.document || document;

  var script = goog.dom.createElement(goog.dom.TagName.SCRIPT);
  var request = {script_: script, timeout_: undefined};
  var deferred = new goog.async.Deferred(goog.tire_iron_.jsloader.cancel_, request);

  // Set a timeout.
  var timeout = null;
  var timeoutDuration = goog.isDefAndNotNull(options.timeout) ?
      options.timeout :
      goog.tire_iron_.jsloader.DEFAULT_TIMEOUT;
  if (timeoutDuration > 0) {
    timeout = window.setTimeout(function() {
      goog.tire_iron_.jsloader.cleanup_(script, true);
      deferred.errback(
          new goog.tire_iron_.jsloader.Error(
              goog.tire_iron_.jsloader.ErrorCode.TIMEOUT,
  ...
  script.onload = script.onreadystatechange = function() {
    if (!script.readyState || script.readyState == 'loaded' ||
      ...
      goog.tire_iron_.jsloader.cleanup_(script, removeScriptNode, timeout);
  ...
  script.onerror = function() {
    goog.tire_iron_.jsloader.cleanup_(script, true, timeout);
    deferred.errback(
        new goog.tire_iron_.jsloader.Error(
            goog.tire_iron_.jsloader.ErrorCode.LOAD_ERROR,
  ...
  var scriptParent = goog.tire_iron_.jsloader.getScriptParentElement_(doc);
 ...
 * @param {goog.tire_iron_.jsloader.Options} options Optional parameters. See
 *     goog.tire_iron_.jsloader.Options documentation for details.
 * @return {!goog.async.Deferred} The deferred result, that may be used to add
 *     callbacks and/or cancel the transmission.
 ...
 *     parameter.
 */
goog.tire_iron_.jsloader.loadAndVerify = function(uri, verificationObjName, options) {
  // Define the global objects variable.
  if (!goog.global[goog.tire_iron_.jsloader.GLOBAL_VERIFY_OBJS_]) {
    goog.global[goog.tire_iron_.jsloader.GLOBAL_VERIFY_OBJS_] = {};
  }
  var verifyObjs = goog.global[goog.tire_iron_.jsloader.GLOBAL_VERIFY_OBJS_];
  ...
  if (goog.isDef(verifyObjs[verificationObjName])) {
    ...
    return goog.async.Deferred.fail(
        new goog.tire_iron_.jsloader.Error(
            goog.tire_iron_.jsloader.ErrorCode.VERIFY_OBJECT_ALREADY_EXISTS,
            'Verification object ' + verificationObjName +
                ' already defined.'));
  }

  // Send request to load the JavaScript.
  var sendDeferred = goog.tire_iron_.jsloader.load(uri, options);
  ...
  sendDeferred.addCallback(function() {
    ...
    if (goog.isDef(result)) {
      ...
      deferred.errback(
          new goog.tire_iron_.jsloader.Error(
              goog.tire_iron_.jsloader.ErrorCode.VERIFY_ERROR, 'Script ' + uri +
 ...
 * @private
 */
goog.tire_iron_.jsloader.getScriptParentElement_ = function(doc) {
 ...
 * @private
 */
goog.tire_iron_.jsloader.cancel_ = function() {
  var request = this;
  if (request && request.script_) {
    var scriptNode = request.script_;
    if (scriptNode && scriptNode.tagName == goog.dom.TagName.SCRIPT) {
      goog.tire_iron_.jsloader.cleanup_(scriptNode, true, request.timeout_);
    }
  }
};
 ...
 * @private
 */
goog.tire_iron_.jsloader.cleanup_ = function(
 ...
 * @enum {number}
 */
goog.tire_iron_.jsloader.ErrorCode = {
  LOAD_ERROR: 0,
  TIMEOUT: 1,
  VERIFY_ERROR: 2,
 ...
 * A jsloader error.
 *
 * @param {goog.tire_iron_.jsloader.ErrorCode} code The error code.
 * @param {string=} opt_message Additional message.
 * @constructor
 * @extends {goog.debug.Error}
 * @final
 */
goog.tire_iron_.jsloader.Error = function(code, opt_message) {
  var msg = 'Jsloader error (code #' + code + ')';
  if (opt_message) {
    msg += ': ' + opt_message;
  }
  goog.tire_iron_.jsloader.Error.base(this, 'constructor', msg);

  /**
   * The code for this error.
   *
   * @type {goog.tire_iron_.jsloader.ErrorCode}
   */
  this.code = code;
};
goog.inherits(goog.tire_iron_.jsloader.Error, goog.debug.Error);
